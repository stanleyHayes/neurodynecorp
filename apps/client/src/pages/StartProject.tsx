      ...
      <Box sx={{ bgcolor: "background.paper", border: "1px solid rgba(108,99,255,.14)" }}>
        ...
        <Box sx={{ p: { xs: 2, md: 4 } }}>
          ...
          {draft.currentSection === 0 && <Stack spacing={2.25} sx={{ mb: 3 }}>
            <FormControl fullWidth><InputLabel>Project category</InputLabel><Select label="Project category" value={draft.category} onChange={e => setDraft(d => ({ ...d, category: e.target.value as ProjectIntakeCategory, currentSection: 0 }))}>{PROJECT_INTAKE_CATEGORIES.map(c => <MenuItem key={c.id} value={c.id}><Box><Typography>{c.label}</Typography><Typography variant="caption" color="text.secondary">{c.description}</Typography></Box></MenuItem>)}</Select></FormControl>
