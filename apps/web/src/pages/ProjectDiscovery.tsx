...
import { Alert, Box, Button, Chip, Container, Dialog, DialogContent, FormControl, InputLabel, LinearProgress, MenuItem, Select, Stack, TextField, Typography } from "@mui/material";
      ...
      <Box sx={{ display: "grid", gridTemplateColumns: { xs: "1fr", lg: "280px minmax(0,1fr)" }, gap: { xs: 3, lg: 5 } }}>
        ...
        <Box sx={{ minWidth: 0, bgcolor: "background.paper", border: "1px solid rgba(108,99,255,.14)", boxShadow: "0 24px 80px rgba(18,18,35,.12)" }}>
          ...
          <Box sx={{ p: { xs: 2.5, sm: 4, md: 6 } }}>
            ...
            <Stack direction={{ xs: "column-reverse", sm: "row" }} sx={{ justifyContent: "space-between", mt: 5, gap: 2 }}><Button disabled={draft.currentSection === 0} onClick={() => setDraft(d => ({ ...d, currentSection: Math.max(0, d.currentSection - 1) }))}>Back</Button><Stack direction={{ xs: "column", sm: "row" }} sx={{ gap: 1 }}><Button startIcon={busy ? <ContentSkeleton compact /> : <SaveOutlinedIcon />} onClick={() => void save()} disabled={busy}>Save and continue later</Button>{draft.currentSection < sections.length - 1 ? <Button variant="contained" onClick={() => setDraft(d => ({ ...d, currentSection: d.currentSection + 1 }))}>Continue</Button> : <Button variant="contained" onClick={() => void submit()} disabled={busy}>Submit project brief</Button>}</Stack></Stack>
          </Box>
        </Box>
      </Box>
    </Container>
    <Dialog open={copilotOpen} onClose={() => setCopilotOpen(false)} fullWidth maxWidth="sm"><DialogContent sx={{ p: { xs: 2.5, sm: 4 } }}><Stack direction="row" sx={{ alignItems: "center", gap: 1.5 }}><AutoAwesomeOutlinedIcon color="primary" /><Box><Typography variant="h5" sx={{ fontWeight: 720 }}>Brief copilot</Typography><Typography variant="body2" color="text.secondary">For {section.title.toLowerCase()}</Typography></Box></Stack><Typography color="text.secondary" sx={{ my: 2 }}>Share a rough thought or ask what you may be missing. The copilot suggests; you decide what belongs in the brief.</Typography><TextField autoFocus fullWidth multiline minRows={4} value={copilotPrompt} onChange={e => setCopilotPrompt(e.target.value)} placeholder="My idea is still rough, but…" />{copilotReply && <Box sx={{ mt: 2, p: 2.5, bgcolor: "rgba(108,99,255,.07)", borderLeft: "2px solid", borderColor: "primary.main", whiteSpace: "pre-wrap" }}><Typography>{copilotReply}</Typography></Box>}<Stack direction="row" sx={{ justifyContent: "flex-end", mt: 2 }}><Button variant="contained" aria-label="Refine my thinking" aria-busy={copilotBusy} onClick={() => void ask()} disabled={copilotBusy || !copilotPrompt.trim()}>{copilotBusy ? <ContentSkeleton compact /> : "Refine my thinking"}</Button></Stack></DialogContent></Dialog>
  </Box>;
}
